...
class ResNet_Oneway(nn.Module):

    def __init__(self, model_org, num_classes):
        super(ResNet_Oneway, self).__init__()

        self.tmp = list(model_org.children());
        self.tmp.pop();
        self.tmp.pop();
        self.tmp.append(DepthToSpace(num_split=num_classes));
        # smoothing
        self.tmp.append(conv_nxn_with_init(in_channels=num_classes,
                                           out_channels=num_classes,
                                           kernel_size=7, stride=1,
                                           padding=3, bias=False));
        self.tmp.append(nn.BatchNorm2d(num_classes));
        self.tmp.append(nn.ReLU(inplace=True));
        self.tmp.append(conv_nxn_with_init(in_channels=num_classes,
                                           out_channels=num_classes,
                                           kernel_size=7, stride=1,
                                           padding=3, bias=False));
...
def resnet50_oneway(num_classes, pretrained=True) :
    model = resnet50(pretrained=pretrained);
    model = ResNet_Oneway(model, num_classes);
    return model
